...
func TestAWSIAMDocuments(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"RedshiftDatabases": {
			...
			statements: []*awslib.Statement{
				{Effect: awslib.EffectAllow, Actions: []string{"redshift:GetClusterCredentials"}, Resources: []string{
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			policy, policyErr := buildPolicyDocument(test.flags, test.fileConfig, test.accountID, test.target)
			boundary, boundaryErr := buildPolicyBoundaryDocument(test.flags, test.fileConfig, test.accountID, test.target)
