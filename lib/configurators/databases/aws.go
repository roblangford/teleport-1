...
func buildActions(config AWSConfiguratorConfig) ([]ConfiguratorAction, error) {
	...
	// Generate policies.
	policy, err := buildPolicyDocument(config.Flags, config.FileConfig, accountID, target)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	policyBoundary, err := buildPolicyBoundaryDocument(config.Flags, config.FileConfig, accountID, target)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	formattedPolicy, err := formatPolicyDocument(policy.Document)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	formattedPolicyBoundary, err := formatPolicyDocument(policyBoundary.Document)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func buildPolicyDocument(flags BootstrapFlags, fileConfig *config.FileConfig, accountID string, target awslib.Identity) (*awslib.Policy, error) {
	var statements []*awslib.Statement
	rdsAutoDiscovery := isRDSAutoDiscoveryEnabled(flags, fileConfig)
	redshiftAutoDiscovery := isRedshiftAutoDiscoveryEnabled(flags, fileConfig)

	if rdsAutoDiscovery {
		statements = append(statements, buildRDSAutoDiscoveryStatements()...)
	}

	if redshiftAutoDiscovery {
		statements = append(statements, buildRedshiftAutoDiscoveryStatements()...)
	}

	// If any of the Auto discovery are enabled, we need permission to edit the
	// target user/role.
	if rdsAutoDiscovery || redshiftAutoDiscovery {
		targetStaments, err := buildIAMAutoDiscoveryStatements(target)
		if err != nil {
			return nil, trace.Wrap(err)
		}
	...
	// Redshift databases permissions.
	redshiftDatabasesStatements, err := buildRedshiftStatements(fileConfig, accountID)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	statements = append(statements, redshiftDatabasesStatements...)
...
func buildPolicyBoundaryDocument(flags BootstrapFlags, fileConfig *config.FileConfig, accountID string, target awslib.Identity) (*awslib.Policy, error) {
	var statements []*awslib.Statement
	rdsAutoDiscovery := isRDSAutoDiscoveryEnabled(flags, fileConfig)
	redshiftAutoDiscovery := isRedshiftAutoDiscoveryEnabled(flags, fileConfig)
	...
	if redshiftAutoDiscovery {
		statements = append(statements, buildRedshiftAutoDiscoveryBoundaryStatements()...)
	}
...
// isRedshiftAutoDiscoveryEnabled checks if the agent needs permission for
// Redshift auto-discovery.
func isRedshiftAutoDiscoveryEnabled(flags BootstrapFlags, fileConfig *config.FileConfig) bool {
...
// buildIAMAutoDiscoveryStatements returns IAM statements necessary when there
// is a AWS databases auto-discovery enabled.
func buildIAMAutoDiscoveryStatements(target awslib.Identity) ([]*awslib.Statement, error) {
...
// buildRedshiftAutoDiscoveryStatements returns IAM statements necessary for
// Redshift databases auto-discovery.
func buildRedshiftAutoDiscoveryStatements() []*awslib.Statement {
	return []*awslib.Statement{
		{
			Effect:    awslib.EffectAllow,
...
// buildRedshiftAutoDiscoveryBoundaryStatements returns IAM boundary statements
// necessary for Redshift databases auto-discovery.
func buildRedshiftAutoDiscoveryBoundaryStatements() []*awslib.Statement {
	return []*awslib.Statement{
		{
			Effect:    awslib.EffectAllow,
...
func buildRedshiftStatements(fileConfig *config.FileConfig, accountID string) ([]*awslib.Statement, error) {
